...
async def transaction_create_request(request: web.BaseRequest):
    ...
    if not jobs:
        raise web.HTTPForbidden(
            reason="The transaction related jobs are not set up in connection metadata for this connection record"
        )
    if "transaction_my_job" not in jobs.keys():
        raise web.HTTPForbidden(
            reason='The "transaction_my_job" is not set in "transaction_jobs" connection metadata for this connection record'
        )
    if "transaction_their_job" not in jobs.keys():
        raise web.HTTPForbidden(
            reason='Ask the other agent to set up "transaction_my_job" in "transaction_jobs" in connection metadata for their connection record'
        )
...
async def set_endorser_info(request: web.BaseRequest):
    ...
    if "transaction_my_job" not in jobs.keys():
        raise web.HTTPForbidden(
            reason='The "transaction_my_job" is not set in "transaction_jobs" in connection metadata for this connection record'
        )
    if jobs["transaction_my_job"] != TransactionJob.TRANSACTION_AUTHOR.name:
        raise web.HTTPForbidden(
            reason="Only a TRANSACTION_AUTHOR can add endorser_info to metadata of its connection record"
        )
    async with context.profile.session() as session:
