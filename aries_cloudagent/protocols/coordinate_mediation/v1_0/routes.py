...
@docs(
    tags=["mediation"],
    summary="Query agent-to-agent mediation",
)
@querystring_schema(MediationsSchema())
@response_schema(MediationsListSchema(), 200)
async def list_mediator_records(request: web.BaseRequest):
    """
...
@match_info_schema(MediationIdSchema())
async def remove_mediation(request: web.BaseRequest):
    """
...
async def register(app: web.Application):
    ...
    app.add_routes(
        [
            web.get("/mediations", list_mediator_records, allow_head=False),
            web.connect("/mediations", send_mediator_request), # send request to other mediator
            web.post("/mediations", receive_mediator_request), # receive a request
            web.update("/mediations", grant_or_deny_mediator_request),
            #web.delete("/mediations", remove_mediator_request),
        ]
    )
